...
**Current state:** Phase A (monetization spine), the rebrand to Settleroo, several rounds of live UAT fixes, and a week of Manus.ai-built tickets (CHR-10 through CHR-28) are merged and live — see `docs/2026-07-27-manus-sync-and-known-gaps.md` for the full sync. Both product gaps it identified are now fixed: frequency/overdue-reminder handling (Round 1) and partial payment + automatic carry-forward (Round 2, see `docs/2026-07-27-partial-payment-carry-forward.md` — utility<->utility only, deliberately not rent). A 4-agent user-research exercise (`docs/2026-07-24-settleroo-integrated-research-synthesis.md` + `docs/2026-07-24-settleroo-strategy-okrs-roadmap-backlog.md`) proposes re-sequencing the roadmap around collection/arrears — **not yet adopted, a separate decision pending Chris's review.** **Phase B (Operator plane + `npm run seed`) is still the next big structural phase**, paused while UAT/trust fixes took priority. Start from the "Phase B" section of the canonical roadmap when resumed. **Build order: B → C → M → D → E** (M = marketing polish, can run in parallel once B is merged) — subject to revision if the collection-first pivot above is adopted.
